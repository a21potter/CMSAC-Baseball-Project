...
library(mgcv)
init_logit_gam <- gam(on_base_binary ~ s(init_logistic_model$fitted.values), data = statcast_training_data, family = "binomial")


init_logistic_model$fitted.values %>%
...
## Run the untransformed logit output through a nonlinear logistic model
...
```{r}
statcast_training_data <- statcast_training_data %>%
  mutate(predict_on_base_prob = 
           as.numeric(predict(init_logit_gam,
                              newdata = statcast_training_data,
                              type = "response")),
         init_gam_onbase_class = as.numeric(predict_on_base_prob >= 0.5))
```

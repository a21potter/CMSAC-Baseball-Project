...
statcast_data_minus_2020_2022 <- statcast_data %>%
  filter(!(game_year %in% c(2015, 2016, 2017, 2020, 2022)))
```
...
```{r}
write_rds(statcast_data_minus_2020_2022, "post-presentation/V2_data/V2_statcast_data_modified.rds", compress = "gz")
```
...
```{r}
statcast_model_data <- statcast_model_data %>%
  mutate(batter_instance_identifier = paste(batter, game_year, stand, if_fielding_alignment, sep = ":")) 

statcast_training_data_rf_identifiers <- statcast_model_data %>%
...
statcast_training_data <- statcast_model_data %>%
  filter(batter_instance_identifier %in% statcast_training_data_rf_identifiers$batter_instance_identifier) %>%
  mutate(is_train = 1)

statcast_testing_data <- statcast_model_data %>%
  filter(!batter_instance_identifier %in% statcast_training_data_rf_identifiers$batter_instance_identifier) %>%
  mutate(is_train = 0)

statcast_train_test_model_data <- rbind(statcast_training_data, statcast_testing_data)
```
...
init_logistic_model <- glm(on_base_binary ~ general_pitch_type + 
                          ...
                          p_throws * stand * if_fielding_alignment, 
                        data = statcast_training_data, 
                        family = "binomial")

#summary(init_logistic_model)
# add fitted vals col
statcast_training_data <- statcast_training_data %>%
  drop_na(delta_woba) %>%
  mutate(init_fitted_values = init_logistic_model$fitted.values)


# 2 step modeling: gam
library(mgcv)
init_logit_gam <- gam(on_base_binary ~ s(statcast_training_data$init_fitted_values), data = statcast_training_data, family = "binomial")

statcast_training_data <- statcast_training_data %>%
  mutate(logit_transformed_output = as.numeric(predict(init_logit_gam, data = ., type = "response")))
...
```{r}
# MODEL OVERALL
logit_model_plot_main <- statcast_training_data %>%
  mutate(bin_pred_prob = round(logit_transformed_output / 0.01) * .01) %>%
...
write_rds(logit_model_plot_main, 
          "data/tables/logit_model_plot_main.rds", compress = "gz")
```



```{r}
logit_model_plot_faceted <- statcast_training_data %>%
  mutate(bin_pred_prob = round(logit_transformed_output / 0.01) * .01) %>%
  # Group by bin_pred_prob:
  group_by(stand, if_fielding_alignment, bin_pred_prob) %>%
  # Calculate the calibration results:
  summarise(n_attempts = n(),
            bin_actual_prob = mean(on_base_binary)) %>%
  ggplot(aes(x = bin_pred_prob, y = bin_actual_prob)) +
  ...
  geom_abline(slope = 1, intercept = 0, 
              color = "black", linetype = "dashed") +
  facet_grid(stand ~ if_fielding_alignment) +
  coord_equal() + 
  scale_x_continuous(limits = c(.2,.5)) + 
  scale_y_continuous(limits = c(.2,.5)) + 
  labs(size = "Number of At-Bats",
...
write_rds(logit_model_plot_faceted, 
          "data/tables/logit_model_plot_faceted.rds", compress = "gz")
```
...
```{r}
statcast_testing_data %>%
  drop_na(delta_woba) %>%
  #mutate(init_fitted_values = predict(init_logistic_model, newdata = ., type = "response")) %>%
  mutate(test_logit_transformed = as.numeric(predict(init_logit_gam, newdata = ., type = "response")))
  
```


## Mutate fitted values from the logit function and the logit_gam

```{r}
statcast_training_data <- statcast_training_data %>%
  drop_na(woba_value) %>%
  mutate(logit_transformed_output = as.numeric(predict(init_logistic_model, newdata = statcast_training_data, type = 'response'))) %>%
  mutate(logit_transformed_output_gam = as.numeric(predict(init_logit_gam, data = statcast_training_data, type = "response")))
```

## Run predictions

```{r}
avg_onbase_binary_outcome <- statcast_training_data %>%
  summarize(mean_onbase_binary = mean(on_base_binary)) %>%
  pull()


statcast_training_data <- statcast_training_data %>%
  mutate(init_gam_onbase_class = as.numeric(logit_transformed_output >= avg_onbase_binary_outcome))
```
...
library(Stat2Data)
emplogitplot1(on_base_binary ~ delta_woba, data = statcast_model_data, 
              ngroups = 10)
```
...
statcast_model_data %>%
  ungroup() %>%
  select(on_base_binary, delta_woba, general_pitch_type, game_year, if_fielding_alignment, zone_simp, stand, p_throws) %>%
  na.omit() %>%

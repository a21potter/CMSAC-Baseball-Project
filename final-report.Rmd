...
logit_model_plot_facted <- read_rds("data/tables/logit_model_plot_faceted.rds")
```
...
![](images/strategic_shift.png){width=50% height=50%}
...
Prior to modeling, we created a more concise way of observing wOBA. 
  ...
  - To weight the delta values by the quantity of plate appearances, we used the harmonic mean:
    
      $$
      H = \frac{2*PA_{shifted}*PA_{non-shifted}} {PA_{shifted} / PA_{non-shifted}}
      $$
      
We also created an indicator to reduce at-bat outcomes to binary.
  
### Linear Modeling - Delta wOBA 
...
To model offensive outcomes at the at-bat level, we used several explanatory variables and interactions:
...
Initially, we were attempting to fit a nonlinear trend with a linear model. In this instance, the model will find the line with the best approximation, but it will be locally biased. The resulting pattern is **sigmoidal**, due to the transformation the results are passed through.

From here, we were able to adjust the model using smooth nonlinear optimization - a GAM model. 
...
A separate question that we came across was looking at the
